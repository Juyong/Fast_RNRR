...
namespace nanoflann {
	...
	struct KDTreeAdaptor {
		...
		{
			out_idxdist.clear();
			/*IndexType out_indices[10];
			num_t out_distances_sq[10];
			query(query_point, 10, out_indices, out_distances_sq);
			for (int i = 0; i < 10; i++)
			{
				if (out_distances_sq[i] < radius)
				{
					out_idxdist.push_back(std::pair<IndexType, num_t>(out_indices[i], out_distances_sq[i]));
				}
				else
					break;
			}
			*/
			//std::cout << "query_point = " << query_point[0] << " " << query_point[1] << " " << query_point[2] << std::endl;
...
typedef nanoflann::KDTreeAdaptor<Eigen::MatrixXd, -1, nanoflann::metric_L2_Simple> KDtree;
