...
double QNwelsch::DoNonRigid()
{
    ...
    double end_nu1 = pars_.Data_endk * std::min(CalcEdgelength(src_mesh_, 0), CalcEdgelength(tar_mesh_, 0));
    double nu2 = pars_.Smooth_nu;
...
#pragma omp parallel for
        ...
        int total_inner_iters = 0;
            ...
            {
				for (int inner_iter = 0; inner_iter < pars_.max_inner_iters; inner_iter++)
				{
                    update_R();
                    ...
                    Smat_X_ = ldlt_->solve(b);
                    total_inner_iters ++ ;
                    if((prev_X - Smat_X_).norm() < pars_.stop)
                    {
                        break;
                    }
				}
...
//            weight_d_.setOnes();
        ...
        if(pars_.print_each_step_info)
        {
            std::string out_obj = pars_.out_each_step_info + "/iter_obj_" + std::to_string(out_iter) + ".obj";
...
//        SimplePruning(correspondence_pairs_, pars_.use_distance_reject, pars_.use_normal_reject);
       ...
       if((curV - prevV).rowwise().norm().maxCoeff() < pars_.stop)
        {
            if(!pars_.use_Dynamic_nu || !pars_.use_welsch_function || nu1 <= end_nu1)
            {
//                std::cout << "break：nu1 = " << nu1 << " nu2 = " << nu2 << std::endl;
                break;
            }    
            else
            {
...
void QNwelsch::LBFGS(int iter, MatrixXX & dir) const
{
    ...
    MatrixXX q(4 * n_src_vertex_, 3);
    MatrixXX temp(4 * n_src_vertex_, 3);
    int k = iter;
    q.setZero();
    dir = q;
    q = -grad_X_;
